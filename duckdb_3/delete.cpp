...
// ============================================================
void file_db_demo(int input_num) {
    DuckDB db(DB_PATH);
    Connection con(db);
    // ファイルDBに書き込む
     cout << "\n=== DELETE ===\n";

    std::string target_id = std::to_string(input_num);
    std::string delete_sql = "DELETE FROM todos WHERE id =" + target_id + ";";
    // --- 1件削除 ---
    exec(con, delete_sql);
    cout << input_num << "を削除しました。\n";
